...
skills <- tbbl$scaled[tbbl$data_set=="2021"][[1]]%>% 
  ...
  select(-neighbor,-sil_width)%>%
  select(NOC, cluster, corrected_cluster, everything())
...
Consider the following biplot.  The arrows represent the projection of the 35 different skills into this 2 dimensional space.  The length of the arrows is proportional to the variance of the skill across NOCs, and the angle between two arrows is proportional to the correlation between the two skills. The fact that many arrows are overlapping suggests a fair degree of redundancy in the skill measures (variables measuring basically the same thing).  Notice that most arrows point south-west (ish), indicating this is where high skill NOCs will be located (e.g. NOCs in clusters 7).  In the south direction the skills are hard (making and fixing things), whereas in the west the skills are soft (managing people). If high skill NOCs are located to the South-West, then low skill NOCs will be found to the North-East (e.g NOCs in cluster 11). 


```{r, fig.height=8, fig.width=8}
corrected_res <- corrected_ave%>%
  column_to_rownames(var="corrected_cluster")%>%
  prcomp()
ggbiplot::ggbiplot(corrected_res, labels = corrected_ave$corrected_cluster, varname.size = 1.5, var.scale = .5)+
  theme_minimal()
```

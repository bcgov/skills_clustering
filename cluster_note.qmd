...
```{r, include=FALSE}
set.seed(123)
cluster_names <- tibble::tibble(corrected_cluster=as.character(1:11), 
                        description=c("Supervisors (services)",
                                      "Managers",
                                      "Investigators",
                                      "Writers",
                                      "Clerks",
...
clValid_wrapper <- function(tbbl, metric){
  results <- clValid(tbbl, nClust = 2:20,  clMethods = c("agnes","diana","kmeans","pam"), metric=metric, validation = "internal")
...
#read in the skills data----------------------
skills_2021 <- read_excel(here::here("data","Skills_27_1_NOC2021_data.xlsx"))%>%
...
screeplot <- fviz_eig(skills_pca) #first 5 components 
importance <- summary(skills_pca)$importance[,1:5]
first_five <- skills_pca[["x"]][,1:5]%>%
  as.data.frame()
#randomize data for comparison
random_2021 <- skills_2021%>%
  mutate(across(everything(), ~runif(length(.x), min(.x), max(.x))))
```


```{r, include=FALSE, cache=TRUE}
tbbl <- tibble(data_set=c("2021", "random_2021", "first_five"), scaled=list(skills_2021, random_2021, first_five))%>%
...
```{r}
tbbl$clValid_euc[tbbl$data_set=="2021"][[1]][24:30]
```

### 35D data, Manhattan distance

```{r}
tbbl$clValid_man[tbbl$data_set=="2021"][[1]][24:30]
```
...
```{r}
tbbl$clValid_euc[tbbl$data_set=="first_five"][[1]][24:30]
```

### 5D data, Manhattan distance

```{r}
tbbl$clValid_man[tbbl$data_set=="first_five"][[1]][24:30]
```
...
plt <- fviz_dend(vis_mod, horiz = TRUE, show_labels = TRUE)
...
```{r, warning=FALSE, fig.height=10}
plt <- fviz_dend(dunn_mod, horiz = TRUE, show_labels = TRUE)
ggplotly(plt, tooltip = "label")%>%
          style(textposition = "right")
vis_dend <- as.dendrogram(vis_mod)
dunn_dend <- as.dendrogram(dunn_mod)
```

## Compare the two models:

The silhouette and dendogram plots of the two models look similar but it would be good if we could quantify the differences between the two dendrograms.  We consider two measures: entanglement @kassambara2017practical and the [cophenetic correlation](https://en.wikipedia.org/wiki/Cophenetic_correlation). 

Entanglement measures the extent to which the labels of the dendrograms match up.  Values close to zero indicate similarity whereas an entanglement of 1 indicates no similarity.  For our two models entanglement is `r round(entanglement(vis_dend, dunn_dend), 2)`, indicating similarity between the two models. 

The cophenetic distance between two observations that have been clustered is defined to be the intergroup dissimilarity at which the two observations are first combined into a single cluster. The cophenetic correlation is the correlation between two cophenetic distance matrices of the two trees.  For our two models the cophenetic correlation is `r round(cor_cophenetic(vis_mod, dunn_mod),2)`, once again indicating similarity between the two models.

## Chosen algorithm:

Recall that for Agnes, 5D data, 11 clusters, Euclidean distance the average silhouette width was `r round(vis_mod$silinfo$avg.width, 2)`, where as for Agnes, 35D data, 11 clusters, Euclidean distance the average silhouette width was `r round(dunn_mod$silinfo$avg.width,2)`. Because a higher average silhouette width is desirable, we proceed with Agnes, 5D data, 11 clusters, Euclidean distance.  Note that even though the average silhouette value is higher, there are still some NOCs that have negative silhouette widths, indicating assignment to the wrong cluster. In the table below we report both the original and corrected cluster assignments, where we move NOCs that have a negative silhouette value to their closest neighbour cluster.  
...
vis_clusters <- vis_sil%>%
  ...
  arrange(sil_width)
DT::datatable(vis_clusters, rownames = FALSE)
```
...
skills <- tbbl$scaled[tbbl$data_set=="2021"][[1]]%>% 
  ...
  full_join(cluster_names)%>%
  select(-neighbor)%>%
  select(NOC, cluster, sil_width, corrected_cluster, description, everything())

corrected_ave <- skills%>%
  group_by(description)%>%
  summarize(across(where(is.numeric), mean))

```

## Biplot of corrected cluster means:

Consider the following biplot.  The arrows represent the projection of the 35 different skills into this 2 dimensional space.  The length of the arrows is proportional to the variance of the skill across NOCs, and the angle between two arrows is proportional to the correlation between the two skills. The fact that many arrows are overlapping suggests a fair degree of redundancy in the skill measures (variables measuring basically the same thing).  Notice that most arrows point south-east (ish), indicating this is where high skill NOCs will be located (e.g. Engineers).  In the south direction the skills are hard (making and fixing things), whereas in the east the skills are soft (managing people). If high skill NOCs are located to the South-East, then low skill NOCs will be found to the North-West (e.g Labourers). 


```{r, fig.height=8, fig.width=8}
corrected_res <- corrected_ave%>%
  column_to_rownames(var="description")%>%
  prcomp()
ggbiplot::ggbiplot(corrected_res, labels = corrected_ave$description, varname.size = 1.5, var.scale = .5, labels.size = 2)+
  theme_minimal()
```
...
skills%>%
  mutate(cluster=as.factor(cluster),
         corrected_cluster=as.factor(corrected_cluster),

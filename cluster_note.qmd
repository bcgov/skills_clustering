...
Consider the following biplot.  The arrows represent the projection of the 35 different skills into this 2 dimensional space.  The length of the arrows is proportional to the variance of the skill across NOCs, and the angle between two arrows is proportional to the correlation between the two skills. The fact that many arrows are overlapping suggests a fair degree of redundancy in the skill measures (variables measuring basically the same thing).  Notice that most arrows point northwest (ish), indicating this is where high skill NOCs will be located (e.g. STEM).  In the north direction the skills are hard (making and fixing things), whereas in the west the skills are soft (managing people). If high skill NOCs are located to the Northwest, then low skill NOCs will be found to the Southeast (e.g Doers). 
...
corrected_res <- corrected_ave%>%
  ...
  prcomp()
plt <- ggbiplot::ggbiplot(corrected_res, labels = corrected_ave$new_cluster, varname.size = 1.5, var.scale = .5, labels.size = 2)+
  theme_minimal()
...
```{r}
cluster_vs_teer <- skills%>%
  mutate(teer=str_sub(NOC, 2,2), .after=NOC)%>%
  tabyl(new_cluster, teer)
colnames(cluster_vs_teer) <- c("Cluster", paste0("TEER: ", 0:5))

DT::datatable(cluster_vs_teer, options=list(dom="t"), rownames = FALSE)
```

Note the strong relationship between our clusters and the TEER clasifications.

```{r, warning=FALSE}
chisq.test(cluster_vs_teer)
```
